...
from benchmark.harness.cases import BenchmarkCaseSpec
from benchmark.utils.hardware_constants import HardwareSpecs, RTX_4050_LAPTOP
...
def _kernel_records_from_group(
    case_spec: BenchmarkCaseSpec,
    metadata: ReportMetadata,
    grouped: dict[str, list[dict]],
    kernel: str,
    ...
    return sorted(grouped.get(kernel, []), key=lambda item: item[axis_key])


def _plot_kernel_metric_lines(
    ax: Any,
    case_spec: BenchmarkCaseSpec,
    metadata: ReportMetadata,
    grouped: dict[str, list[dict]],
    ...
    for kernel in metadata.kernels:
        kernel_records = _kernel_records_from_group(case_spec, metadata, grouped, kernel)
        if not kernel_records:
            continue
        style = kernel_line_style(case_spec.operation, kernel)
        ax.plot(
            [record[axis_key] for record in kernel_records],
            [record[metric_key] for record in kernel_records],
            color=style["color"],
            marker=style["marker"],
            lw=style["lw"],
            ms=style["ms"],
...
def _metric_bar_points(
    ...
    axis_key = case_spec.get_axis(metadata.sweep_axis).parameter
    kernel_records = _sorted_kernel_records(case_spec, records, kernel, metadata.sweep_axis)
    values = []
    positions = []
    for idx, x_value in enumerate(x_values):
        candidates = [
            ...
            if int(record[axis_key]) == int(x_value)
        ]
        if not candidates:
            continue
        record = candidates[0]
        values.append(record[metric_key])
        positions.append(float(x_positions[idx] + offset))
    return values, positions


def _final_dimension_records(
    case_spec: BenchmarkCaseSpec,
    metadata: ReportMetadata,
    records: list[dict],
) -> list[dict]:
    if not metadata.x_values:
        return []

    axis_key = case_spec.get_axis(metadata.sweep_axis).parameter
    final_x_value = int(metadata.x_values[-1])
    lookup = _record_lookup(case_spec, records, axis_key)
    final_records: list[dict] = []
    for kernel in metadata.kernels:
        kernel_record = lookup.get(kernel, {}).get(final_x_value)
        if kernel_record is not None:
            final_records.append(kernel_record)
    return final_records


def _naive_reference_kernel(metadata: ReportMetadata) -> str | None:
    for kernel in metadata.kernels:
        if kernel.startswith("naive_"):
            return kernel
    return metadata.kernels[0] if metadata.kernels else None


def _format_final_dimension_table(
    case_spec: BenchmarkCaseSpec,
    metadata: ReportMetadata,
    records: list[dict],
) -> str:
    final_records = _final_dimension_records(case_spec, metadata, records)
    if not final_records:
        return "No records available for final sweep dimension.\n"

    baseline_record = next(
        (record for record in final_records if record["kernel"] == metadata.baseline_kernel),
        None,
    )
    naive_kernel = _naive_reference_kernel(metadata)
    naive_record = next(
        (record for record in final_records if record["kernel"] == naive_kernel),
        None,
    )
    baseline_tflops = (
        float(baseline_record["achieved_tflops"])
        if baseline_record is not None and baseline_record["achieved_tflops"] > 0
        else None
    )
    naive_time = (
        float(naive_record["median_ms"])
        if naive_record is not None and naive_record["median_ms"] > 0
        else None
    )
    ...
    for record in final_records:
        kernel = resolve_kernel_name(case_spec.operation, record["kernel"])
        time_ms = float(record["median_ms"])
        tflops = float(record["achieved_tflops"])
        bandwidth = float(record["achieved_bw"])
        vs_pytorch = (
            f"{(tflops / baseline_tflops) * 100.0:.1f}%"
            if baseline_tflops is not None
            else "--"
        )
        speedup = (
            f"{naive_time / time_ms:.1f}x"
            if naive_time is not None and time_ms > 0
            else "--"
        )
        rows.append(
            (
                kernel_label(case_spec.operation, kernel),
                f"{time_ms:.2f}",
                f"{tflops:.2f}",
                f"{bandwidth:.2f}",
                vs_pytorch,
    ...
    def _line(values: tuple[str, ...]) -> str:
        return "  ".join(
            value.ljust(widths[idx]) if idx == 0 else value.rjust(widths[idx])
            for idx, value in enumerate(values)
        )

    axis_key = case_spec.get_axis(metadata.sweep_axis).parameter
    final_x_value = int(final_records[0][axis_key])
    lines = [
        f"Final sweep summary for {case_spec.operation} at {metadata.sweep_axis}={final_x_value}",
        _line(headers),
        _line(tuple("-" * width for width in widths)),
    ]
...
def write_final_dimension_table(
    case_spec: BenchmarkCaseSpec,
    metadata: ReportMetadata,
    records: list[dict],
    out_path: Path,
) -> Path:
    out_path.write_text(_format_final_dimension_table(case_spec, metadata, records))
    return out_path
...
def legacy_sweep_report(
    ...
    txt_path = write_final_dimension_table(
        case_spec,
        metadata,
        records,
        timing_dir / f"{stem}_final.txt",
    )
    ...
    ax0.set_yscale("log")
    _plot_kernel_metric_lines(ax0, case_spec, metadata, grouped, "median_ms")
    ...
    _plot_kernel_metric_lines(ax1, case_spec, metadata, grouped, "achieved_tflops")
    ...
    _plot_kernel_metric_lines(ax2, case_spec, metadata, grouped, "achieved_bw")
    ...
    if not _plot_speedup_lines(ax3, case_spec, metadata, records):
        ax3.text(
            0.5,
            0.5,
    ...
    return [json_path, txt_path, plot_path]
...
def legacy_compare_report(
    ...
    active_kernels = _active_kernels(case_spec, metadata, records)
    ...
    for metric_name in metrics:
        ...
        for idx, kernel in enumerate(active_kernels):
            values, positions = _metric_bar_points(
                case_spec=case_spec,
                metadata=metadata,
                records=records,
                kernel=kernel,
