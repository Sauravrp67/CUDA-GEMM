#!/usr/bin/env python3
"""Minimal Nsight Compute target application backed by GEMM case specs."""
...
from benchmark.profiling.common import (
...
def _find_stack_size() -> int | None:
    ...
    status = cudart.cudaDeviceGetLimit(ctypes.byref(stack_size), cuda_limit_stack_size)
    if status != 0:
        return None
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Single-launch profiler target for Nsight Compute. Do not add timing loops here."
    )
...
def main() -> None:
    ...
    shape_desc = " ".join(f"{key}={params[key]}" for key in ("M", "K", "N") if key in params)
    print(
