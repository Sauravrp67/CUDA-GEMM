...
__global__ void warptile_register2DTiledSgemm(const float* A, const int M, const int K, const int lda, bool transA,
        ...
        /* Subtile constants Layout
        Derived from Siboehm's Blog: https://siboehm.com/articles/22/CUDA-MMM
        We have W_TILE_WIDTH_M x W_TILE_WIDTH_N region, which must handled by 32 threads.
        We divide W_TILE_WIDTH_M x W_TILE_WIDTH_N into n subtiles each with dimension WSUBM x WSUBN. 
        In WSUMM x WSUBN subtile region, each thread owns TMxTN regions. 
        If WSUM x WSUBN is to be handled by 32 threads in a warp, then:
        num_thread = (WSUM x WSUBN) / (TN x TM)
        num_thread = ((W_TILE_WIDTH_M / WMITER) x (W_TILE_WIDTH_N / WNITER)) / (TN x TM)
        WMITER x WNITER = (W_TILE_WIDTH_M x W_TILE_WIDTH_N) / (num_thread x TM x TN)
        num_thread = warpSize
        */

        // This also gives us the position of the subtile in a warptile
        ...
        // A warp's thread placement inside each subtile.
        ...
        for(size_t ph = 0; ph < (K + TILE_WIDTH_K - 1) / TILE_WIDTH_K; ph++){
            ...
            for(size_t tid = linear_id; tid < TILE_WIDTH_K * TILE_WIDTH_M / 4; tid += num_threads){
                ...
                uint smem_row_A = tid / (TILE_WIDTH_K / 4); // varies from 0 - 63(for BM = 64, BK = 8)
                uint smem_col_A = tid % (TILE_WIDTH_K / 4); // varies from 0 - 1
                ...
                if(global_row < M && (global_k + 3) < K){
                    ...
                    for(size_t lane = 0; lane < 4; ++lane){
                        ...
                        Mds[smem_col_A * 4 + lane][smem_row_A] = 
                            (global_row < M && k_lane < K)
                                ? A[global_row * lda + k_lane]
                                : 0.0f;
                    }
                }
            }
                ...
                if((global_col + 3) < N && (global_k) < K){
                    ...
                    for(size_t lane = 0; lane < 4; ++lane){
                        ...
                        Nds[smem_row_B][smem_col_B * 4 + lane] = 
                            (col_lane < N && global_k < K)
                            ? B[global_k * ldb + col_lane]
                            : 0.0f;
                    }
                }
            }
            ...
            // Now onto compute phase. Partial Sums of outer product via threadTiling.

            for(size_t k = 0; k < TILE_WIDTH_K; k++){
                
                // Load Values from Mds to register Tiles warptiling and Thread Tiling.
                for(size_t subTile_row = 0; subTile_row < warp_M_iter; ++subTile_row){
                    
                        float4 tempAs = reinterpret_cast<const float4*>(&Mds[k][global_row_warp_offset + subTile_row * warp_SUB_M + threadIdx_row_InWarp * 4])[0];
                ...
                // Load Values from Nds to register Tiles warptiling and Thread Tiling 
                for(size_t subTile_col = 0; subTile_col < warp_N_iter; ++subTile_col){
                    
                        float4 tempBs = reinterpret_cast<const float4*>(&Nds[k][global_col_warp_offset + subTile_col * warp_SUB_N + threadIdx_col_InWarp * 4])[0];
        ...
        //Writing the blockTile and warptiled result back to C
                ...
                for(size_t i = 0; i < THREAD_M; ++i){
                    for(size_t j = 0; j < THREAD_N; j += 4){
                        ...
                        uint global_cRow = 
                            ...
                            + global_row_warp_offset 
                            + subTile_row * warp_SUB_M 
                            + threadIdx_row_InWarp * THREAD_M + i;
                        uint global_cCol = 
                            global_col_block_offset 
                            + global_col_warp_offset 
                            + subTile_col * warp_SUB_N 
                            + threadIdx_col_InWarp * THREAD_N + j; 

                        // When the M dimension is very small like 16-32 incase of smaller input dimension.
                        if(global_cRow >= M){
                            continue;
                        }
